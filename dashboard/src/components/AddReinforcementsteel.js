...
const AddReinforcementsteel = () => {
    ...
    const handleAddMullion = async (e) => {
        e.preventDefault();
        console.log({ name, code, thickness, lengthOfBeamId, weightPermeter, pricePermeter, profileId });

        try {
            if (!name || !code || !weightPermeter || !thickness || !pricePermeter || !profileId) {
                toast.error("Please fill in all required fields");
                return;
            }

            const res = await axios.post(
                "https://your-server-url.com/api/v1/Reinforcementsteel",
                { name, code, thickness, Length_of_Beam: lengthOfBeamId, weightPermeter, pricePermeter, profile: profileId },
                {
