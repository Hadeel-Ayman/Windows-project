...
const AddGlazingBead = () => {
    ...
    const handleAddMullion = async (e) => {
        e.preventDefault();
        console.log({ name, code, lengthOfBeamId, height, weightPermeter, coloursId, pricePermeter, profileId });

        try {
            if (!name || !code || !height || !weightPermeter || !pricePermeter || !profileId) {
                toast.error("Please fill in all required fields");
                return;
            }

            const res = await axios.post(
                "https://your-server-url.com/api/v1/GlazingBead",
                { name, code, Length_of_Beam: lengthOfBeamId, height: height, weightPermeter, colours: coloursId, pricePermeter, profile: profileId },
                {
